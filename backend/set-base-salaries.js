...
async function setBaseSalaries() {
  try {
    ...
    // Get all users with 0 or undefined base salary
    ...
    console.log(`\n👥 Found ${users.length} users without base salary set\n`);
    ...
    for (const user of users) {
      ...
      const defaultSalary = salaryConfig[salaryKey] || 0;
      
      console.log(`Updating ${user.username} (${user.role}): ₱${user.baseSalary || 0} → ₱${defaultSalary}`);
      
      // Update user base salary
      user.baseSalary = defaultSalary;
      await user.save();
      ...
      if (payroll) {
        payroll.baseSalary = defaultSalary;
        payroll.totalSalary = defaultSalary + (payroll.over || 0) - (payroll.short || 0) - (payroll.deduction || 0) - (payroll.withdrawal || 0);
        await payroll.save();
