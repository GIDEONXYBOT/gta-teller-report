...
async function main() {
  try {
    ...
    if (args.length < 2) {
      console.error('Usage: node check-reports-created-range.js <startDate> <endDate>');
      process.exit(1);
    }
    ...
    const count = await TR.find({ createdAt: { $gte: startDay, $lte: endDay } }).countDocuments();
    console.log(`Teller reports created between ${startDay.toISOString()} and ${endDay.toISOString()}: ${count}`);

    const samples = await TR.find({ createdAt: { $gte: startDay, $lte: endDay } }).limit(10).lean();
    console.log('Sample created reports (max 10):', samples.map(s => ({ _id: s._id, tellerId: s.tellerId, createdAt: s.createdAt, date: s.date, over: s.over, short: s.short })));
