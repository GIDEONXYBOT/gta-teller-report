...
import { CheckCircle2, Clock, XCircle, FileSpreadsheet } from "lucide-react";
...
export default function AdminSupervisorReport() {
  ...
  function handleExportToGoogleSheets() {
    if (!report || !report.tellers || report.tellers.length === 0) {
    ...
    try {
      ...
      const rows = report.tellers.map(t => [
        t.tellerName || "",
      ...
      // Add totals row
      const totalSystemBalance = report.tellers.reduce((sum, t) => sum + Number(t.systemBalance || 0), 0);
      const totalCashOnHand = report.tellers.reduce((sum, t) => sum + Number(t.cashOnHand || 0), 0);
      const totalShort = report.tellers.reduce((sum, t) => sum + Number(t.short || 0), 0);
      const totalOver = report.tellers.reduce((sum, t) => sum + Number(t.over || 0), 0);

      rows.push(["TOTAL", totalSystemBalance, totalCashOnHand, totalShort, totalOver]);

      // Convert to CSV
      const csvContent = [
        [`Supervisor Report - ${report.supervisorName}`],
      ...
      copyToClipboard().then(success => {
        if (!success) {
          ...
          link.setAttribute("download", `supervisor_report_${report.supervisorName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={!report || !report.tellers || report.tellers.length === 0}
            className={`flex items-center gap-2 px-3 py-1 rounded text-white ${
              !report || !report.tellers || report.tellers.length === 0
