...
export default function AdminTellerOfMonth() {
  const { settings } = useContext(SettingsContext);
  const theme = settings?.theme || {};
  const darkMode = settings?.theme?.mode === "dark";

  const [tellers, setTellers] = useState([]); // ✅ renamed + safe array
      ...
      try {
        const res = await axios.get(
          import.meta.env.VITE_API_URL
            ? `${import.meta.env.VITE_API_URL}/api/employee/monthly`
            : "http://localhost:5000/api/employee/monthly"
        );
  ...
  const handleSavePrizes = async () => {
    try {
      await axios.post(
        import.meta.env.VITE_API_URL
          ? `${import.meta.env.VITE_API_URL}/api/employee/prizes`
          : "http://localhost:5000/api/employee/prizes",
        prizes
      );
  ...
  const handleReset = async () => {
    ...
    try {
      await axios.delete(
        import.meta.env.VITE_API_URL
          ? `${import.meta.env.VITE_API_URL}/api/employee/reset`
          : "http://localhost:5000/api/employee/reset"
      );
      alert("✅ Teller data reset for new month!");
      setTellers([]);
    } catch (err) {
